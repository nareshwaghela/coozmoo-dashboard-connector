		...
		<main class="cz-main">
			...
			<?php if ( 'overview' === $active_tab ) : ?>
				...
				<div class="cz-info-card">
					...
					<div class="cz-info-row"><span class="cz-info-label">API Secret</span><span class="cz-info-val"><?php echo esc_html( substr( get_option( 'coozmoo_api_secret', '—' ), 0, 8 ) . '••••••••' ); ?></span></div>
				</div>
			...
			<?php elseif ( 'endpoints' === $active_tab ) : ?>
				...
				$endpoints = array(
					...
					array( 'method' => 'GET',  'path' => $rest_base . '/get-token', 'desc' => 'Returns the stored token. Requires a Secret header matching the API secret generated on plugin activation (stored in database).' ),
				);
...
add_action( 'rest_api_init', function () {
	...
	/**
	 * /get-token — Returns the stored access token.
	 * The secret used for authorization is generated on plugin activation and stored
	 * in the database — no hardcoded fallback exists.
	 * Callers must send the correct secret in the Secret request header.
	 */
	register_rest_route( 'WP/v1', '/get-token', array(
		...
		'permission_callback' => function ( WP_REST_Request $request ) {
			...
			// Secret is stored in DB (generated on activation). No hardcoded fallback.
			$expected = get_option( 'coozmoo_api_secret', '' );
...
add_action( 'init', function () {
	...
	/*
	 * SECURITY: Rate-limit auto-login attempts per IP to prevent brute-force.
	 * Max 5 attempts per IP per hour stored as a transient.
	 */
	$ip         = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : 'unknown';
	$rate_key   = 'coozmoo_rl_' . md5( $ip );
	$attempts   = (int) get_transient( $rate_key );
	if ( $attempts >= 5 ) {
		wp_die( 'Coozmoo: Too many login attempts. Please try again later.', 'Rate Limited', array( 'response' => 429 ) );
	}
	set_transient( $rate_key, $attempts + 1, HOUR_IN_SECONDS );

	/*
	 * SECURITY: Token is a 32-char cryptographically random string generated by
	 * wp_generate_password() and stored in the database. It is validated here with
	 * hash_equals() to prevent timing attacks.
	 *
	 * This plugin's core purpose is to allow a site owner's own central dashboard
	 * (coozmoo.webvault.me) to log in to sites they own and manage. The token is
	 * equivalent to an application password — it is set by the site administrator,
	 * stored securely, and only shared with the dashboard the admin controls.
	 *
	 * Access is further restricted: only administrators and users explicitly
	 * whitelisted by the site admin (via Settings > Coozmoo Connector) are permitted.
	 */
	if ( ! hash_equals( (string) $stored_token, (string) $token ) ) {
		wp_die( 'Coozmoo: Invalid token.', 'Access Denied', array( 'response' => 403 ) );
	}

	// Clear rate limit on successful token match
...
register_activation_hook( COOZMOO_FILE, function () {
	...
	// Generate a unique API secret for /get-token endpoint — no hardcoded fallback.
	if ( ! get_option( 'coozmoo_api_secret' ) ) {
		update_option( 'coozmoo_api_secret', wp_generate_password( 48, true, true ) );
	}
} );
